...
def validate_manifest(manifest_path: str, schema_path: str, samples_dir: str):
    ...
    valid_categories = {"logo", "icon", "illustration", "complex_artwork", "photograph", "binary_graphic"}
    ...
    with open(manifest_file, "r", encoding="utf-8") as f:
        ...
        for row_idx, row in enumerate(reader, start=2):
            ...
            if not filepath.exists():
                report["errors"].append(f"Row {row_idx}: File '{filename}' not found in samples directory.")
                has_error = True
                continue
                
            actual_sha256 = get_sha256(str(filepath))
            if expected_sha256 and actual_sha256 != expected_sha256:
                report["errors"].append(f"Row {row_idx}: SHA-256 mismatch for '{filename}'. Expected: {expected_sha256}, Got: {actual_sha256}")
                has_error = True
            ...
            if actual_sha256 in seen_hashes:
                ...
                report["duplicates"].append({"file1": dup, "file2": filename, "sha256": actual_sha256})
            else:
                seen_hashes[actual_sha256] = filename
                
            width, height, has_alpha, is_corrupt = get_image_info(str(filepath))
            if is_corrupt:
                report["errors"].append(f"Row {row_idx}: File '{filename}' is corrupted or unreadable.")
                has_error = True
                continue
            ...
            if expected_w and str(width) != expected_w:
                report["errors"].append(f"Row {row_idx}: Width mismatch for '{filename}'. Expected: {expected_w}, Got: {width}")
                has_error = True
            if expected_h and str(height) != expected_h:
                report["errors"].append(f"Row {row_idx}: Height mismatch for '{filename}'. Expected: {expected_h}, Got: {height}")
                has_error = True
                
            if has_alpha_str == "true" and not has_alpha:
                report["errors"].append(f"Row {row_idx}: Metadata says has_alpha=true, but no alpha channel found in '{filename}'.")
                has_error = True
            elif has_alpha_str == "false" and has_alpha:
                report["warnings"].append(f"Row {row_idx}: Metadata says has_alpha=false, but alpha channel detected in '{filename}'.")
