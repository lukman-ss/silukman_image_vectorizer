...
from benchmark.baselines.backends import SilukmanBackend, VTracerBackend, PotraceBackend, InkscapeBackend
...
class ExperimentRunner:
    ...
    def _initialize_backends(self) -> dict:
        ...
        for b_name in self.config.backends:
            ...
            if b_name not in registry:
                print(f"Warning: Unknown backend '{b_name}'. Skipping.")
                continue
                
            backend = registry[b_name]
            if not backend.is_available():
                print(f"Warning: Backend '{b_name}' is not available on this system. Skipping.")
                continue
        ...
        if resume_id:
            ...
            if len(parts) >= 4:
                ...
                if old_hash != self.config_hash:
                    raise ValueError(f"Config hash mismatch! Original: {old_hash}, Current: {self.config_hash}. Cannot resume with modified config.")
            return resume_id
    ...
    def _load_dataset(self) -> List[dict]:
        ...
        with open(manifest_path, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                if row.get("split") == self.config.dataset.split and row.get("category") in self.config.dataset.categories:
                    dataset.append(row)
        return dataset
            ...
            with open(self.runs_file, 'r', encoding='utf-8') as f:
                for line in f:
                    ...
                    try:
                        ...
                        print("Warning: Detected truncated or invalid JSONL line during resume recovery. Ignoring line.")
    ...
    def execute(self):
        ...
        if not dataset:
            print("Dataset empty for the given filters.")
            return
        ...
        total_tasks = len(dataset) * len(self.backends) * len(self.config.presets) * self.config.experiment.repetitions
        completed = 0
        success_count = 0
        fail_count = 0
        
        print(f"Starting execution: {total_tasks} total tasks.")
                ...
                for b_name, backend in self.backends.items():
                    for preset in self.config.presets:
                        for rep in range(1, self.config.experiment.repetitions + 1):
                            ...
                            print(f"[{completed+1}/{total_tasks}] Running {run_id} ...")
                            ...
                            # 2. Evaluation
                            if not is_skipped and vectorize_result.get("performance", {}).get("success", False):
                                ...
                                base_record["output_hash"] = calculate_file_hash(output_path) if os.path.exists(output_path) else None
                                ...
                                err = vectorize_result.get("error") or vectorize_result.get("performance", {}).get("error")
                                if err:
