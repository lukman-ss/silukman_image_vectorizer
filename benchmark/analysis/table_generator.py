    ...
    def generate_overall_benchmark(self):
        ...
        for backend, presets in self.agg_data.get("overall", {}).items():
            for preset, stats in presets.items():
                ...
                r["Success Rate (%)"] = round(
                    stats["runs"]["success"] / max(stats["runs"]["total"], 1) * 100, 2
                )
                ...
                # Quality
                if "ssim" in metrics:
                    r["SSIM (Median)"] = round(metrics["ssim"].get("median", 0), 4)
                if "rmse" in metrics:
                    r["RMSE (Median)"] = round(metrics["rmse"].get("median", 0), 2)
                if "edge_f1" in metrics:
                    r["Edge F1 (Median)"] = round(metrics["edge_f1"].get("median", 0), 4)

                # Complexity
                if "svg_bytes" in metrics:
                    r["SVG Size KB (Median)"] = round(
                        metrics["svg_bytes"].get("median", 0) / 1024, 2
                    )
                if "path_count" in metrics:
                    r["Paths (Median)"] = metrics["path_count"].get("median", 0)

                # Runtime
                if "wall_clock_time_seconds" in metrics:
                    r["Runtime s (Median)"] = round(
                        metrics["wall_clock_time_seconds"].get("median", 0), 3
                    )
                ...
                records.append(
                    {
                        ...
                        "Count": data["count"],
                        "% of Total Failures": round(
                            data["count"] / max(fail_data["total_failures"], 1) * 100, 2
                        ),
                    }
                )
