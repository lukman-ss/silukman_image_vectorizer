...
HIGHER_IS_BETTER = {
    "psnr", "ssim", "histogram_correlation", "edge_f1", 
    "edge_precision", "edge_recall", "success"
}
...
class PairedComparison:
    ...
    def __init__(self, aggregated_json_path: str):
        ...
        if "by_image" not in self.data:
            raise ValueError("Aggregated data must contain 'by_image' grouping for paired analysis.")
    ...
    def compare(self, config_a: Tuple[str, str], config_b: Tuple[str, str], metric: str) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "significance_test": {
                ...
                "p_value": p_value,
                "note": "Not run if sample size < 10 or zero variance" if not test_valid else "Valid"
            },
            "per_image_deltas": per_image_details
        }

    def save_report(self, config_a: Tuple[str, str], config_b: Tuple[str, str], metrics: List[str], output_path: str):
        report = {}
        for m in metrics:
