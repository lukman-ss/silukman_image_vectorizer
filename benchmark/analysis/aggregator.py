...
class BenchmarkAggregator:
    ...
    def _calculate_stats(self, values: List[float]) -> Dict[str, Any]:
        ...
        if n == 0:
            return {"count": 0, "mean": None, "median": None, "std": None, "min": None, "max": None, "p25": None, "p75": None, "ci_95": None}
    ...
    def aggregate(self) -> Dict[str, Any]:
        ...
        status_bpc = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: {"total": 0, "success": 0, "failed": 0, "skipped": 0})))
        status_bp = defaultdict(lambda: defaultdict(lambda: {"total": 0, "success": 0, "failed": 0, "skipped": 0}))
        status_bpi = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: {"total": 0, "success": 0, "failed": 0, "skipped": 0})))
        ...
        for record in self.raw_data:
            ...
            metrics = {}
            if "quality" in record: metrics.update(record["quality"])
            if "complexity" in record: metrics.update(record["complexity"])
            if "performance" in record: metrics.update(record["performance"])
        ...
        for b, presets in by_bpi.items():
            if b not in report["by_image"]: report["by_image"][b] = {}
            for p, images in presets.items():
                report["by_image"][b][p] = {}
                for i, metrics in images.items():
                    rep = {
                        "runs": status_bpi[b][p][i],
                        "metrics": {}
                    }
                    for m_key, vals in metrics.items():
                        rep["metrics"][m_key] = self._calculate_stats(vals)
