...
class VectorizationConfig:
    ...
    palette_replacements: List[Tuple[Tuple[int, int, int], Tuple[int, int, int]]] = field(
    ...
    def _validate_choices(field_name: str, value: Any, choices: list):
        if value not in choices:
            raise ValueError(
                f"Invalid value for {field_name}: '{value}'. Must be one of {choices}."
            )
    ...
    def _validate_range(field_name: str, value: Any, min_val: float, max_val: float):
        if not (min_val <= value <= max_val):
            raise ValueError(
                f"Invalid value for {field_name}: {value}. Must be between {min_val} and {max_val}."
            )
