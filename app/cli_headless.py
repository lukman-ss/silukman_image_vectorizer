...
def _build_parser() -> argparse.ArgumentParser:
    ...
    parser_vectorize.add_argument("--preset", "-p", help="Preset name to use (default: balanced)", default="balanced")
    parser_vectorize.add_argument("--config", "-c", help="Path to custom JSON config file (overrides preset)")
    parser_vectorize.add_argument("--json", action="store_true", help="Output results as JSON")
    parser_vectorize.add_argument("--dry-run", action="store_true", help="Simulate without executing")
    ...
    parser_batch.add_argument("--output-dir", "-o", required=True, help="Path to the output directory for SVGs")
    parser_batch.add_argument("--preset", "-p", help="Preset name to use (default: balanced)", default="balanced")
    parser_batch.add_argument("--config", "-c", help="Path to custom JSON config file (overrides preset)")
    parser_batch.add_argument("--json", action="store_true", help="Output results as JSON")
    parser_batch.add_argument("--workers", type=int, default=2, help="Number of parallel workers (default: 2)")
    parser_batch.add_argument("--resume", action="store_true", help="Skip files already present in output")
    parser_batch.add_argument("--overwrite", action="store_true", help="Overwrite existing output files")
    parser_batch.add_argument("--ext", help="Comma-separated extensions to include (e.g., .png,.jpg)")
...
def _load_settings(preset_name: str, config_path: str = None) -> VectorizationSettings:
    if config_path:
...
def cmd_batch(args: argparse.Namespace) -> int:
    ...
    files = sorted([f for f in input_dir.iterdir() if f.is_file() and f.suffix.lower() in supported_exts])
