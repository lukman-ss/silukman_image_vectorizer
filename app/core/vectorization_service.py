...
from app.core.exceptions import InvalidInputError, ProcessingError, ExportError
...
def vectorize_image(
    ...
    from app.core.preprocessing import preprocess_image, apply_grayscale_threshold
    ...
    if not os.path.exists(input_path):
        result.error_type = "InvalidInputError"
    ...
    try:
        ...
        _finalize_result(result, start_time_perf)
        raise ExportError(result.error_message)
    ...
    if img_info is None:
        ...
        _finalize_result(result, start_time_perf)
        raise InvalidInputError(result.error_message)
    ...
    try:
        ...
        if not cv2.imwrite(temp_input_png_path, preprocessed_img):
            raise ProcessingError("Failed to write temporary preprocessed image.")

        thresholded_array = None
        if config.engine_type != "VTracer":
            thresholded_array, meta = apply_grayscale_threshold(preprocessed_img, config.threshold_val)
        ...
        if getattr(vector_result, "fallback_error", None):
            warnings.append(f"VTracer failed, fell back to OpenCV Legacy. Error: {vector_result.fallback_error}")
            
    except Exception as e:
        result.error_type = type(e).__name__
        result.error_message = str(e)
        _finalize_result(result, start_time_perf)
        raise ProcessingError(f"Vectorization failed: {str(e)}") from e
    finally:
    ...
    try:
        ...
        final_output_path = export_svg(vector_result, str(normalized_output_path), source_filename=source_name)
    except Exception as e:
        result.error_type = "ExportError"
        result.error_message = str(e)
        _finalize_result(result, start_time_perf)
