...
def test_cli_valid_vectorization(synth_image, tmp_path):
    ...
    res = subprocess.run(CLI + ["vectorize", synth_image, "-o", out], capture_output=True, text=True)
    assert res.returncode == 0
...
def test_cli_json_output(synth_image, tmp_path):
    ...
    res = subprocess.run(CLI + ["vectorize", synth_image, "-o", out, "--json"], capture_output=True, text=True)
    assert res.returncode == 0
...
def test_cli_dry_run(synth_image, tmp_path):
    ...
    res = subprocess.run(CLI + ["vectorize", synth_image, "-o", out, "--dry-run"], capture_output=True, text=True)
    assert res.returncode == 0
...
def test_cli_batch(synth_image, tmp_path):
    ...
    res = subprocess.run(CLI + ["batch", str(in_dir), "-o", str(out_dir), "--json"], capture_output=True, text=True)
    assert res.returncode == 0
    data = json.loads(res.stdout)
...
def test_benchmark_config_validation(tmp_path):
    # Invalid config path
    res = subprocess.run(CLI + ["benchmark", "run", "-c", "invalid.yaml"], capture_output=True, text=True)
    assert res.returncode != 0
