...
from app.cli_headless import _build_parser, main
...
def test_parser_commands(parser):
    ...
    args = parser.parse_args(
        ["vectorize", "input.png", "--output", "out.svg", "--preset", "high_fidelity"]
    )
...
def test_cmd_inspect(mock_open, mock_parse, mock_calc, capsys):
    mock_root = MagicMock()
    mock_root.get.side_effect = lambda key, default=None: (
        "800" if key == "width" else "600" if key == "height" else default
    )
    mock_parse.return_value = mock_root
    mock_calc.return_value = {"path_count": 10, "total_elements": 15}
...
@patch("builtins.open", new_callable=MagicMock)
def test_cmd_batch_success(
    mock_open, mock_mkdir, mock_is_dir, mock_iterdir, mock_load, mock_vec, capsys
):
